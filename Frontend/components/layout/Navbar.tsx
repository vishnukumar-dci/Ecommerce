...
export default function Navbar() {
  ...
  useEffect(() => {
    ...
    const filtered = allProducts
      .filter((p) => String(p.product_name || p.name).toLowerCase().includes(query))
      .slice(0, 5);
    ...
    <header className="sticky top-0 z-50 bg-white/80 backdrop-blur-md border-b border-slate-100/80 shadow-sm transition-all duration-300">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3.5 flex items-center justify-between gap-4">
        ...
        <div className="flex-shrink-0 flex items-center gap-2">
          <Link href="/" className="font-extrabold text-2xl tracking-tight bg-gradient-to-r from-blue-700 to-indigo-600 bg-clip-text text-transparent transition-all duration-300 hover:opacity-90">
            ShopEZ
          </Link>
        </div>
            ...
            <div className="absolute top-full left-0 right-0 mt-1 bg-white border border-slate-100 shadow-xl rounded-2xl overflow-hidden z-50 animate-in fade-in duration-100">
              {suggestions.map((s) => (
                ...
                >
                  ...
                  <span className="text-sm text-slate-800 font-medium">{s.product_name || s.name}</span>
                </div>
              ))}
            </div>
          )}
        ...
        <nav className="hidden md:flex items-center gap-5 text-sm font-semibold text-slate-600">
          <Link href="/products" className="flex items-center gap-1.5 px-3 py-2 rounded-xl hover:bg-slate-50 hover:text-blue-700 transition-all duration-200"> 
            <Grid size={18}/>
            <span>Shop</span>
          </Link>
          
          {/* Wishlist Icon */}
          <Link href="/profile?tab=wishlist" className="relative flex items-center gap-1.5 px-3 py-2 rounded-xl hover:bg-slate-50 hover:text-blue-700 transition-all duration-200">
            <Heart size={18} className="text-slate-500 hover:text-red-500 transition-colors" />
            <span>Wishlist</span>
          ...
          {/* Cart Icon */}
          <Link href="/cart" className={`relative flex items-center gap-1.5 px-3 py-2 rounded-xl hover:bg-slate-50 hover:text-blue-700 transition-all duration-200 ${isAdmin ? "hidden" : ""}`}>
          ...
          {/* Logs for Admin */}
          {isAdmin && (
            <Link href="/logs" className="flex items-center gap-1.5 px-3 py-2 rounded-xl hover:bg-slate-50 hover:text-blue-700 transition-all duration-200">
              <History size={18} />
              <span>Logs</span>
            </Link>
          )}
            ...
            {role ? (
              <div>
                ...
                >
                  ...
                  <span className="max-w-[80px] truncate">{name || "User"}</span>
        ...
        <div className="flex md:hidden items-center gap-3">
          ...
          <Link href="/profile?tab=wishlist" className="relative p-2 text-slate-500">
            <Heart size={20} />
            {mounted && wishlistCount > 0 && (
